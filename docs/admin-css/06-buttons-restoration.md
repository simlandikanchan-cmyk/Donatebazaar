...
- Legacy button family (`.action-btn`, `.btn-primary`, `.bulk-btn`, `.export-btn`, `.cp-btn-clear`, `.gc-action-btn`, `.modal-y-btn`, etc.) with shared state rules
- Legacy variant mappings via CSS custom properties (`--btn-bg`, `--btn-fg`, etc.)
...
After restoration, the now-identical top-level `_buttons.css` was deleted as the duplicate.

## Verification

- `npm run build` succeeded; built `public/build/assets/admin-Bgvzs2_L.css` (247.1 kB) — CSS output size matches the pre-incident build (~246 kB + later additions).
- `final_check.ps1` found no undefined classes; every `btn-*` class used by views resolves.
- The `x-button` component (`resources/views/components/button.blade.php`) emits `btn btn--{variant} btn--{size}` — all covered by the restored file.
