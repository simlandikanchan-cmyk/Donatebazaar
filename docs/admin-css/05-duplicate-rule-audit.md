...
The `conflict_check.ps1` script compared selectors across old flat files and the new tree. An additional pairwise diff (`Compare-Object` over `.class` regex extraction) verified old flat files against their new counterparts.
...
| `_variables.css` (top-level) | **kept** | this IS the imported `_variables.css` |
...
| `_buttons.css` (top-level) | **deleted after restore** | content copied 1:1 to `components/_buttons.css` (see `06-buttons-restoration.md`) |
| `components/_buttons.css` (old duplicate) | **deleted** | replaced by the restored canonical file |
