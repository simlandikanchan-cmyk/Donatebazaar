...
<span style="display:none;font-size:1px;color:#eef1f8;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">
    Your blog "{{ $blog->title }}" has been {{ $status === 'published' ? 'approved' : 'rejected' }}.
</span>
  ...
  <tr>
    <td style="background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e9ecf5;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td bgcolor="{{ $status === 'published' ? '#059669' : '#dc2626' }}" style="background-image:{{ $status === 'published' ? 'linear-gradient(135deg,#059669,#34d399)' : 'linear-gradient(135deg,#dc2626,#f87171)' }};background-color:{{ $status === 'published' ? '#059669' : '#dc2626' }};padding:38px 30px 34px;text-align:center;">
            <table role="presentation" align="center" cellpadding="0" cellspacing="0" style="margin:0 auto 16px;">
              <tr><td style="width:54px;height:54px;border-radius:50%;background:rgba(255,255,255,.16);text-align:center;vertical-align:middle;font-size:26px;line-height:54px;color:#ffffff;">{{ $status === 'published' ? '✓' : '✕' }}</td></tr>
            </table>
            <p style="color:#ffffff;font-size:21px;font-weight:700;margin:0 0 4px;">
              Blog {{ $status === 'published' ? 'Approved' : 'Rejected' }}
            </p>
            <p style="color:rgba(255,255,255,.7);font-size:13px;margin:0;">
              {{ $status === 'published' ? 'Your blog is now published' : 'Action needed' }}
            </p>
          </td>
        </tr>
          ...
          <td style="padding:34px 30px 8px;">
            ...
            @if($status === 'published')
              <p style="font-size:14.5px;color:#6b7280;line-height:1.7;margin:0 0 24px;">
                Great news! Your blog <strong>"{{ $blog->title }}"</strong> has been reviewed and
                <strong style="color:#059669;">approved</strong>. It is now published and visible to readers.
              </p>
            @else
              <p style="font-size:14.5px;color:#6b7280;line-height:1.7;margin:0 0 24px;">
                Unfortunately, your blog <strong>"{{ $blog->title }}"</strong> has been
              ...
              <tr>
                <td style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:14px;padding:20px 22px;">
                  ...
                  <p style="color:#475569;font-size:13.5px;margin:0;">
                    Status: <span style="color:{{ $status === 'published' ? '#059669' : '#dc2626' }};font-weight:600;">{{ ucfirst($status) }}</span>
                  </p>
                </td>
              </tr>
            </table>
