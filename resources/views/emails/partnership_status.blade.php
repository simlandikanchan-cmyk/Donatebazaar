        ...
        <tr>
          <td bgcolor="{{ $isApproved ? '#059669' : '#dc2626' }}" style="background-image:{{ $isApproved ? 'linear-gradient(135deg,#059669,#34d399)' : 'linear-gradient(135deg,#dc2626,#f87171)' }};background-color:{{ $isApproved ? '#059669' : '#dc2626' }};padding:38px 30px 34px;text-align:center;">
            ...
            <p style="color:#ffffff;font-size:21px;font-weight:700;margin:0 0 4px;">
              Partnership {{ ucfirst($partnership->status) }}
            </p>
            <p style="color:rgba(255,255,255,.8);font-size:13px;margin:0;">
              {{ $isApproved ? 'Your request has been approved' : 'Update on your request' }}
            </p>
          </td>
        </tr>
          ...
          <td style="padding:34px 30px 8px;">
            ...
            @if($isApproved)
              <p style="font-size:14.5px;color:#6b7280;line-height:1.7;margin:0 0 24px;">
                Great news! Your partnership request with <strong>{{ $partnership->organization_name }}</strong>
                has been reviewed and <strong style="color:#059669;">approved</strong>. Our team will be in touch
                shortly to discuss next steps.
              </p>
            @else
              <p style="font-size:14.5px;color:#6b7280;line-height:1.7;margin:0 0 24px;">
                Thank you for your interest in partnering with {{ config('app.name') }}. After review,
                we're unable to move forward with your request at this time.
              </p>
            @endif
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr>
                <td style="background:{{ $isApproved ? '#f8fafc' : '#fef2f2' }};border:1px solid {{ $isApproved ? '#e5e7eb' : '#fecaca' }};border-radius:14px;padding:20px 22px;">
                  ...
                  <p style="color:{{ $isApproved ? '#475569' : '#b91c1c' }};font-size:14px;margin:0;line-height:1.6;">{{ $partnership->admin_notes }}</p>
                </td>
              </tr>
            </table>
            @endif
                ...
                <td style="padding:18px 20px 0;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td style="padding:12px 0;font-size:13.5px;color:#111827;font-weight:600;">{{ $partnership->organization_name }}</td>
                    </tr>
                  </table>
                </td>
              </tr>
                  ...
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td style="padding:12px 0;">
                        <span style="color:{{ $isApproved ? '#059669' : '#dc2626' }};font-weight:600;font-size:13.5px;">{{ ucfirst($partnership->status) }}</span>
                      </td>
                    </tr>
                  </table>
                </td>
