...
/* actions */
.act-wrap{display:flex;gap:6px;align-items:center;flex-wrap:wrap;}
...
.ev-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--r);padding:16px;box-shadow:var(--sh);margin-bottom:12px;animation:fadeUp .4s ease both;}
              ...
              <div class="act-wrap">
                ...
                @if($event->status === 'pending')
                  <form class="act-form" method="POST" action="{{ route('admin.events.approve', $event->id) }}" onsubmit="return confirm('Approve and publish this event?')">
                    @csrf
                    <button type="submit" class="act-link act-approve">
                  ...
                  <form class="act-form" method="POST" action="{{ route('admin.events.reject', $event->id) }}" onsubmit="return confirm('Reject this event?')">
                    @csrf
                    <button type="submit" class="act-link act-reject">
...
<div class="ev-cards">
  @forelse($events as $event)
    @php
      ...
      $pct    = $goal > 0 ? min(100, round($raised / $goal * 100)) : 0;
    @endphp
    <div class="ev-card" data-name="{{ strtolower($event->title) }}">
